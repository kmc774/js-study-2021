...
<section class="board_section">
    <div class="board_form">
        ...
        <label class="b_num">게시글번호 : <input type="text" class="board_bdIdx view_input" id="bdIdx" name="bdIdx"
                                            value="${board.bdIdx}" readonly></label>
        ...
        <label>내용</label>
        <textarea rows="20" cols="50" placeholder="내용을 입력하세요." class="board_val" id="veiw_textarea"
        ...
        <div class="userId_section">
            <label>작성자 : <input type="text" class="d_userId" name="userId" id="d_userId" value="${board.userId}"
                                readonly>
                <label>등록일 : <input type="text" class="d_regDt" name="regDt" id="d_regDt" value="${board.regDt}"
                                    readonly></label>
                <c:if test="${!empty board.crtnDt}">
                <label>수정일 : <input type="text" class="d_crtnDt" name="crtnDt" id="d_crtnDt" value="${board.crtnDt}"
                                    readonly></label>
                </c:if>
        </div>
        ...
        <div class="btn_section">
            <button type="button" class="search_btn"
                    onclick="location.href='/board/go-update?bdIdx=${board.bdIdx}&page=${paging.page}&cntPerPage=${paging.cntPerPage}&type=${paging.type}&keyword=${paging.keyword}&filter=${paging.filter}'">
                수정하기
            </button>
            <button type="button" class="search_btn"
                    onclick="location.href='/board/list?page=${paging.page}&cntPerPage=${paging.cntPerPage}&type=${paging.type}&keyword=${paging.keyword}&filter=${paging.filter}'">
                목록보기
            </button>
